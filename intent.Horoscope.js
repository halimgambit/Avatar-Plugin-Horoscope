...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules.Horoscope.rules) {	 
		match = (0, _helpers.syntax)(state.sentence, Config.modules.Horoscope.rules[rule]); 	
		if (match) break;
	}
	
	if (match) { // vérifié !
		if (state.debug) info('Intent Horoscope, Syntax: true');
		state.isIntent = true;
